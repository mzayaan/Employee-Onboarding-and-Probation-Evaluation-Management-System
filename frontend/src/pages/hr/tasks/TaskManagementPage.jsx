// =============================================================================
// src/pages/hr/tasks/TaskManagementPage.jsx
// HR assigns tasks to employees and monitors all task assignments.
// Kanban layout matching Figma 12:28: To Do / In Progress / Completed columns.
// FR-07, FR-08 | NFR-03 | Objective 1
// =============================================================================
...
// ── Assign Task Modal ─────────────────────────────────────────────────────────
function AssignTaskModal({ employees, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.profile_id || !form.title.trim() || !form.due_date) {
      setError('Employee, title and due date are required.')
      return
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <h2 className="text-base font-semibold text-slate-800">Assign New Task</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <select value={form.profile_id} onChange={set('profile_id')} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100">
              <option value="">— Select employee —</option>
              {employees.map((e) => (
                <option key={e.profile_id} value={e.profile_id}>
                  {e.user?.first_name} {e.user?.last_name} — {e.job_title || 'No title'}
                </option>
              ))}
            </select>
          </div>
          <div>
            ...
            <input type="text" value={form.title} onChange={set('title')} placeholder="e.g. Complete IT equipment setup"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.priority} onChange={set('priority')} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100">
          ...
          <div className="flex items-center justify-end gap-3 pt-1">
            <button type="button" onClick={onClose} className="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Cancel</button>
            <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 rounded-lg px-5 py-2 text-sm font-medium text-white disabled:opacity-60" style={{ backgroundColor: '#1e3a5f' }}>
...
// ── Kanban Card ───────────────────────────────────────────────────────────────
function TaskCard({ assignment, onDelete, deleting }) {
...
export default function TaskManagementPage() {
  ...
  const fetchData = () => {
    setLoading(true)
    Promise.all([getAllAssignments(), getEmployees()])
      .then(([tasks, emps]) => { setAssignments(tasks); setEmployees(emps) })
  ...
  return (
    <AppShell>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: '#0f1c2e' }}>Task Management</h1>
          <p className="mt-1 text-sm text-slate-500">Assign and monitor onboarding tasks for employees.</p>
        </div>
        <button
